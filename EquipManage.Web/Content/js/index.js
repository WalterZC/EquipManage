...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        ...
        if (row.FParentId == "0") {
            ...
            _html += '<a data-id="' + row.FId + '" href="#" class="nav-link nav-toggle"><i class="' + row.FIcon + '"></i><span class="title">' + row.FFullName + '</span><span class="arrow"></span></a>';
            var childNodes = row.ChildNodes;
            if (childNodes.length > 0) {
                _html += '<ul class="sub-menu nav-second-level">';
                $.each(childNodes, function (i) {
                    var subrow = childNodes[i];
                    _html += '<li class="title">';
                    _html += '<a class="J_menuItem"' + ' id=' + subrow.FId + ' data-id="' + subrow.FId + '" href="' + subrow.FUrlAddress + '" data-index="' + subrow.FSortCode + '">' + subrow.FFullName + '</a>';
                    _html += '</li>';
                });
                _html += '</ul>';
            }
